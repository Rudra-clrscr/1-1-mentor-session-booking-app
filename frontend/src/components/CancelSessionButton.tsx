...
export default function CancelSessionButton({
  ...
  const handleCancel = async () => {
    ...
    setError(null);
    try {
      await apiClient.cancelSession(sessionId, reason.trim() || undefined);
      setShowDialog(false);
      onCancelled();
    } catch (err: unknown) {
      const msg =
  ...
  return (
    <>
      <button
        ...
        className="text-sm text-red-400 hover:text-red-300 underline transition-colors mt-2"
      >
        Cancel session
      </button>
            ...
            {withinWindow ? (
              <>
                <p className="text-red-500 text-sm mb-4">
                  It&apos;s too late to cancel online — this session starts in less than{' '}
              ...
              <>
                <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">
                  Cancellations must be made at least {MIN_NOTICE_HOURS} hours before the session.
                  Both participants will be notified by email.
                </p>
